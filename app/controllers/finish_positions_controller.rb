class FinishPositionsController < ApplicationController
  def create
    ...
    if @finish_position.save
      redirect_to dashboard_path, notice: "#{@finish_position.user.name} placed at ##{@finish_position.position}"
    else
      redirect_to dashboard_path, alert: "Can't add #{@finish_position.user.name} at ##{@finish_position.position}... #{@finish_position.errors.full_messages.to_sentence}"
    end
  end
  ...
  def destroy
    ...
    if @finish_position.destroy
      redirect_to dashboard_path, notice: "#{@finish_position.user.name} removed from ##{@finish_position.position}"
    else
